...
### A22. 词表组标题必须写成 `### ① …`，否则只拿到 12pt 🔴

2026-09-24 第五份稿子踩到。分组标题写成**普通段落** `① 恋爱与关系` 时，
`doc_insert_markdown` 把它当**正文段**处理 → **12pt / 不加粗 / 首行缩进 2 字符**；
写成 **`### ① 恋爱与关系`**（三级标题）才会落成 **16pt 加粗 + 缩进归零**。

**判据（不用打开文件就能发现）**：

- `set_indent.py` 段落分类的 **HEAD 数**：3 组词表 + 大标题 + 一、 + 二、 → 应为 **6**；
  若只有 3，就是组标题掉进了 `P` 类。
- `verify_docx.py` 的 `sz 取值` 应包含 `'32'`（16pt）。
- **最快的办法：拿同系列已定稿的稿子对比**组标题的 `<w:sz>` / `<w:ind>` / `<w:b>`。

**处理**：中间稿里补上 `### `，重跑 `build_docx.py` 即可（不用手工补格式）。

⚠️ **连带缺陷（已修）**：`build_docx.py` 的 `check_markdown` 里分组统计原为
`^[①②③④⑤⑥⑦⑧⑨]`，认不出 `### ① …`，会打印「词表：**0 组**」。
**只影响统计打印，不影响生成。** 现已改为 `^#*\s*[①②③④⑤⑥⑦⑧⑨]`。

### A23. 正文加粗是**独立一步**，`build_docx.py` 不含它 🔴

2026-09-24 第五份稿子漏掉：成品正文一个加粗都没有。

正确顺序：

```bash
bold_terms.py 中间稿.md --terms <词表>.txt -o 加粗版.md
build_docx.py 加粗版.md <out.docx>
```

- **漏掉的信号**：成品**加粗 run 数只有 40 上下**（时间戳 16 + 大标题 1 + 小标题 6 + 词条 22）；
  正常应 **80+**。
- 词表文件**一行一个正则片段**，要匹配**正文里的实际形态**而非词表原形：
  `rush\w* into commitment`、`cram\w* for the exam`、`lean\w* into`、`subsid\w*`、
  `pac\w* (our|your)sel\w*`、`on hold`、`correlat\w* to`
- 加粗后自查：`[A-Za-z]\*\*[A-Za-z]` 出现次数应为 **0**（防相邻加粗吞空格）。
...
- **词表组标题的 `w:sz` 是 `32`（16pt）且 `<w:ind w:firstLineChars="0"/>`**（见 A22）——
  否则说明中间稿里漏写了 `### `
- **加粗 run 数应在 80 以上** —— 只有 40 上下说明漏跑了 `bold_terms.py`（见 A23）
