...
规范要求头部标签（频道 / 原视频链接 / 视频时长 / 整理日期）**不加粗**，
但 markdown 没有"取消加粗"的写法，`**频道：**` 会落成 `<w:b w:val="1"/>`。
上一份成品里是 `<w:b w:val="0"/>`（4 处）—— 那是事后改的。
**处理**：导入后把第 2~4 段的 `<w:b w:val="1"/>` 全部换成 `val="0"`。
`verify_docx.py` 会检查 `<w:b w:val="0"/>` 出现 ≥3 次。
