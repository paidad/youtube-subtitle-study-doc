...
为什么需要这个脚本（三件事 markdown 都表达不出来）：
  ...
  3. 头部标签写成 `**频道：**` 会落成 `<w:b w:val="1"/>`（加粗），
...
头部标签行数**自动探测**（2026-09-24 起头部是条件性的）：
  给了原视频链接 → 3 行（频道 / 原视频链接 / 视频时长…整理日期）
  没给链接       → 1 行（整理日期）
...
def main():
    ...
    # 头部标签行数自动探测：从第 2 段起，连续匹配「频道/原视频链接/视频时长/整理日期：」
    if args.labels is None:
        ...
        while 1 + n < len(ptexts) and HDR.match(ptexts[1 + n].strip()):
            n += 1
        args.labels = n
        shape = '含链接：频道 / 链接 / 时长' if n >= 3 else '无链接：仅整理日期'
